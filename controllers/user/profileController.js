...
const updateProfile = async (req, res) => {
    try {
        ...
        const updatedUser = await users.findByIdAndUpdate({ _id: userId }, { $set: { name: name, mobile: mobile } })
    ...
    try {
        ...
        if (user) {
            ...
            if (!passwordMatch) {
                return res
                    .status(400)
                    .json({ error: 'Password does not match your current password. Please try again.' });
            } else {
...
const editAddress = async (req, res) => {
    try {
        const { addressIndex, fullName, houseName, street, city, state, postalCode } = req.body;
        const userId = req.session.userData._id;
...
const viewWallet = async (req, res) => {
    try {
        ...
        if (!userWallet) {
            return res.render('myWallet', { balance: 0 });
        }

        res.render('myWallet', { balance: userWallet.balance });
