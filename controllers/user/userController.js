  ...
  new GoogleStrategy(
    {
      ...
      callbackURL: "https://herbonline.shop/auth/google/callback",
    },
...
const registerUser = async (req, res) => {
  ...
  try {
    ...
    if (userExits) {
      ...
      if (!userStatus) {
        res.render("userRegistration", {
          message: "Your sign up has been failed",
        });
      }
    }
  } catch (error) {
  ...
  try {
    ...
    transporter.sendMail(mailOptions, (error, info) => {
      if (error) {
        console.log("Error:", error);
        res.render("verifyOTP", {
          message: "Failed to send OTP. Please resend otp",
        });
      } else {
        console.log("Email sent:", info.response);
        res.render("verifyOTP", {
  ...
  try {
    ...
    if (user) {
      ...
      const mailOptions = {
        ...
        <p>Please click this <a href="https://www.herbonline.shop/changePassword">link</a> to reset your password.</p>
      ...
      transporter.sendMail(mailOptions, (error, info) => {
        if (error) {
          ...
          res.render("userLogin", {
            message: "Link will be sent to your email",
          });
        }
      });
    } else {
  ...
  try {
    ...
    if (!user.is_Blocked) {
      ...
      if (totalPrice < 1000) {
        return res.json({
          isValid: false,
          message: "For orders over 1000, enjoy special benefits!",
        });
      }
