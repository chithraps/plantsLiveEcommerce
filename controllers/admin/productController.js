...
const viewProducts = async (req, res) => {
    try {
        ...
        res.render('viewProducts', { products: products, currentPage: page, totalPages })
    } catch (error) {
        console.log(error.message)
    }
}
...
const addProduct = async (req, res) => {
    try {
        ...
        const categories = await category.find();
        if (existingProduct) {
            res.render('addProduct', { categories: categories, message: "Product already exists" })
        } else {
            ...
            if (productStatus) {
                ...
                res.render('addProduct', { categories: categories, message: "Product added successfully" })
            } else {
                res.render('addProduct', { categories: categories, message: "Failed to add product" })
            }
        }
...
const deleteProduct = async (req, res) => {
    try {
        ...
        if (productData.is_Delelted) {
            const updateProduct = await product.updateOne({ _id: productId }, { $set: { is_Delelted: false } })
        } else {
            const updateProduct = await product.updateOne({ _id: productId }, { $set: { is_Delelted: true } })
        }
